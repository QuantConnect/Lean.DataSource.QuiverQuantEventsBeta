...
namespace QuantConnect.DataProcessing
{
    ...
    public class Program
    {
        ...
        public static int Main()
        {
            ...
            QuiverEventsBetaDataDownloader instance;
            try
            {
                // Pass in the values we got from the configuration into the downloader/converter.
                instance = new QuiverEventsBetaDataDownloader(destinationDataFolder);
            }
            catch (Exception err)
            {
                Log.Error(err, $"The downloader/converter for {QuiverEventsBetaDataDownloader.VendorDataName} {QuiverEventsBetaDataDownloader.VendorDataName} data failed to be constructed");
                return 1;
            }
            ...
            try
            {
                ...
                if (!success)
                {
                    Log.Error($"QuantConnect.DataProcessing.Program.Main(): Failed to download/process {QuiverEventsBetaDataDownloader.VendorName} {QuiverEventsBetaDataDownloader.VendorDataName} data");
                    return 1;
                }
            }
            catch (Exception err)
            {
                Log.Error(err, $"The downloader/converter for {QuiverEventsBetaDataDownloader.VendorDataName} {QuiverEventsBetaDataDownloader.VendorDataName} data exited unexpectedly");
                return 1;
            }
            finally
            {
