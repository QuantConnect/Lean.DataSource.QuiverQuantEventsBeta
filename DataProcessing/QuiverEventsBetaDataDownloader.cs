...
{
    /// <summary>
    /// QuiverEventsBetaDataDownloader implementation. https://www.quiverquant.com/
    /// </summary>
    public class QuiverEventsBetaDataDownloader : IDisposable
    {
        public const string VendorName = "quiver";
        public const string VendorDataName = "eventsbeta";
        ...
        /// <summary>
        /// Creates a new instance of <see cref="QuiverEventsBeta"/>
        /// </summary>
        ...
        public QuiverEventsBetaDataDownloader(string destinationDataFolder, string apiKey = null)
        {
        ...
        public bool Run()
        {
            ...
            try
            {
                ...
                Log.Trace($"QuiverEventsBetaDataDownloader.Run(): Start processing {count.ToStringInvariant()} companies");
                ...
                foreach (var company in companies)
                {
                    ...
                    {
                        Log.Error($"QuiverEventsBetaDataDownloader(): Defunct ticker {quiverTicker} is unable to be parsed. Continuing...");
                        continue;
                    }

                    // Begin processing ticker with a normalized value
                    Log.Trace($"QuiverEventsBetaDataDownloader.Run(): Processing {ticker}");

                    // Makes sure we don't overrun Quiver rate limits accidentally
                    _indexGate.WaitToProceed();

                    tasks.Add(
                        HttpRequester($"historical/eventsbeta/{ticker}")
                            .ContinueWith(
                                y =>
                                {
                                    i++;
                                    ...
                                    {
                                        Log.Error(
                                            $"QuiverEventsBetaDataDownloader.Run(): Failed to get data for {company}");
                                        return;
                                    }
                                    ...
                                    var politicalBetas = JsonConvert.DeserializeObject<List<QuiverEventsBeta>>(result, _jsonSerializerSettings);
                                    var csvContents = new List<string>();

                                    foreach (var politicalEvent in politicalBetas)
                                    {
                                        if (politicalEvent.Date.Date == today)
                                        {
                                    ...
                                    {
                                        Log.Trace($"QuiverEventsBetaDataDownloader.Run(): {percentageDone.ToStringInvariant("P2")} complete");
            ...
            Log.Trace($"QuiverEventsBetaDataDownloader.Run(): Finished in {stopwatch.Elapsed.ToStringInvariant(null)}");
            return true;
        }
            ...
            for (var retries = 1; retries <= _maxRetries; retries++)
            {
                try
                {
                    using (var client = new HttpClient())
                    {
                        ...
                        {
                            Log.Error($"QuiverEventsBetaDataDownloader.HttpRequester(): Files not found at url: {Uri.EscapeUriString(url)}");
                ...
                {
                    Log.Error(e, $"QuiverEventsBetaDataDownloader.HttpRequester(): Error at HttpRequester. (retry {retries}/{_maxRetries})");
                    Thread.Sleep(1000);
                }
            }
        ...
        private void SaveContentToFile(string destinationFolder, string ticker, IEnumerable<string> contents)
        {
            ...
            if (finalFileExists)
            {
                Log.Trace($"QuiverEventsBetaDataDownloader.SaveContentToFile(): Adding to existing file: {finalPath}");
            ...
            {
                Log.Trace($"QuiverEventsBetaDataDownloader.SaveContentToFile(): Writing to file: {finalPath}");
            }
